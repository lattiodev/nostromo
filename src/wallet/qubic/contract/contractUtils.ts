...
export interface QHelper {
  getIdentityBytes(value: string): Uint8Array;
  getIdStringFromBytes?(bytes: Uint8Array): string;
}
...
export const CONTRACT_INDEXES: ContractIndexes = {
  QX: 1,
  Qx: 1,
  QUOTTERY: 2,
  Quottery: 2,
...
export function getContractIndex(contractName: string, customIndexes: ContractIndexes | null = null): number {
  const indexes = customIndexes || CONTRACT_INDEXES;
  const upperContractName = contractName.toUpperCase();
  const contractsFound = Object.keys(indexes).filter((key) => key.toUpperCase() === upperContractName);
...
function parseParameters(content: string, functionName: string, direction: "input" | "output"): FunctionParameter[] {
  ...
  if (structMatch) {
    ...
    const fieldRegex =
      /(?:(Array<([a-zA-Z0-9_]+)\s*,\s*([a-zA-Z0-9_]+)>)|([a-zA-Z0-9_]+(?:\[\d+\])?))\s+([a-zA-Z0-9_]+)\s*;/g;
    let fieldMatch: RegExpExecArray | null;
    const fields: FunctionParameter[] = [];

    while ((fieldMatch = fieldRegex.exec(structBody)) !== null) {
      if (fieldMatch[1]) {
        // Matched Array<type, size>
        fields.push({
          type: "Array",
...
// Helper function to safely convert unknown values to bigint
...
function encodeValue(value: unknown, type: string, qHelper?: QHelper, size: number = 0): Buffer {
  let buffer: Buffer;
  const isAmountField = false; // Assuming amount scaling is handled elsewhere
  const scaleAmount = (val: unknown): bigint => {
    if (typeof val === "number" || typeof val === "string" || typeof val === "bigint" || typeof val === "boolean") {
      return BigInt(val) * 1000000n;
    }
    return 0n;
  };
  ...
  if (type === "uint8") {
    ...
    buffer.writeUInt8(safeNumber(value), 0);
  } else if (type === "uint16") {
    buffer = Buffer.alloc(2);
    buffer.writeUInt16LE(safeNumber(value), 0);
  } else if (type === "uint32") {
    buffer = Buffer.alloc(4);
    buffer.writeUint32LE(safeNumber(value), 0);
  } else if (type === "uint64") {
    buffer = Buffer.alloc(8);
    const valToEncode = isAmountField ? scaleAmount(value) : safeBigInt(value);
    ...
    buffer = Buffer.alloc(1);
    buffer.writeInt8(safeNumber(value), 0);
  } else if (type === "int16" || type === "sint16") {
    buffer = Buffer.alloc(2);
    buffer.writeInt16LE(safeNumber(value), 0);
  } else if (type === "int32" || type === "sint32") {
    buffer = Buffer.alloc(4);
    buffer.writeInt32LE(safeNumber(value), 0);
    ...
    if (!qHelper) {
      ...
      try {
        const idBytes = qHelper.getIdentityBytes(String(value)); // Use qHelper method
...
export function encodeParams(params: EncodedParams, inputFields: FunctionParameter[] = [], qHelper?: QHelper): string {
  try {
    ...
    const resolveSize = (sizeStr: string): number => {
      if (/^\d+$/.test(sizeStr)) return parseInt(sizeStr, 10);
      // Add known constants here
      const knownConstants: { [key: string]: number } = {
    ...
    inputFields.forEach((field) => {
      ...
      if (field.type === "Array") {
        ...
        const arraySize = resolveSize(field.size || "0");
        if (arraySize === 0) {
        ...
        for (let i = 0; i < arraySize; i++) {
          ...
          const itemBuffer = encodeValue(itemValue, field.elementType || "uint64", qHelper);
...
function decodeValue(dv: DataView, offset: number, type: string, size: number = 0): DecodeValueResult {
  let value: unknown;
  let fieldSize = 0;
...
export function decodeContractResponse(responseData: string, outputFields?: FunctionParameter[]): DecodedResponse {
  if (!responseData) {
    return {
      success: true,
...
function decodeGenericResponse(buffer: Buffer, dv: DataView, hexDump: string): DecodedResponse {
  const result: DecodedField = {};
...
export const byteArrayToHexString = (byteArray: Uint8Array | null): string => {
  if (!byteArray) return "";
...
export function base64ToUint8Array(base64String: string): Uint8Array {
  try {
    const binaryString = atob(base64String); // Use built-in atob
    ...
    console.error("Error decoding Base64 string:", e, "Input:", base64String.substring(0, 50) + "...");
...
export function decodeUint8ArrayTx(tx: Uint8Array): QubicTransaction {
  ...
  try {
    ...
    throw new Error(
      `Failed to decode transaction structure: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
